...
\section{Caso d'uso}
L'idea alla base di questo progetto è quella di fornire all'utente la ricostruzione in realt\`a virtuale di un supermercato e di svolgere una semplice funzione riabilitativa, quale ``Prendi una bottiglia di Coca-Cola dallo scaffale''. Questa scena deve essere ``costruita'' dal terapista, il quale in base alla lesione neurologica del paziente, al tipo di riabilitazione neurologica decisa con lo staff medico, conoscendo le preferenze ed i gusti del paziente, decider\`a di realizzare l'ambientazione maggiormente appropriata, ponendo nello scaffale gli oggetti disponibili nella libreria acceduta dall'ambiente di backoffice. 
Come prototipo è stato modellato con Blender un minimarket e alcuni dei relativi prodotti. Nelle figure ~\ref{label:marketbrout} e ~\ref{label:marketbrin} si possono vedere due rappresentazioni dell'ambiente creato.

\begin{figure}[Ht]
\centering
\includegraphics[width=0.7\textwidth]{marketbrout.png}
\caption{Rappresentazione del Supermercato: esterno}
...
\caption{Rappresentazione del Supermercato:  interno}
\label{label:marketbrin}
\end{figure}

L'interfaccia di backoffice \`e rappresentata dalla pagina Web, dalla quale tutti i contenuti dovranno essere fruibili. La pagina \`e costituita da tre componenti fondamentali: il canvas per la scena 3D, un controller per la navigazione, lo spostamento, rotazione e rimozione degli oggetti, e la lista degli oggetti presenti in libreria da poter inserire nella scena.  In Figura ~\ref{label:mockupui} si può vedere una bozza dell'interfaccia utente finale. 
Una volta realizzato l'ambiente desiderato, il terapista effettuer\`a l'export della scena X3D che costituisce l'esercizio che il paziente dovr\`a svolgere per la riabilitazione neurologica.
 L'interfaccia realizzata mira ad essere semplice ed immediata per il terapista, operando alcune scelte fondamentali:
\begin{itemize}
    \item Utilizzo del Drag 'n Drop HTML5 per l'inserimento degli oggetti di libreria nella scena;
    \item Controller SVG semplificato ed intuitivo per lo spostamento degli oggetti nella scena.
\end{itemize}
...
\begin{mylisting}{Java}{Medea client: il Drag'n Drop}{lst:medeajs4}
...
La Figura ~\ref{label:medeaui01} mostra l'interfaccia utente finale del software Medea, mentre in Figura ~\ref{label:medeaui02} è rappresentato un utente in fase di posizionamento dell'oggetto selezionato.
%\clearpage
